...
class VWAP(QCAlgorithm):
    ...
    def OnData(self, data):
        if self.currentDayOpenPrice is None:
    ...
    def ShouldEnterToBuy(self, price):
        return (not self.lastBrokenCandle is None
            and not self.lastBrokenCandle is self.windowMinute[0]
    ...
    def UpdateLastBrokenCandle(self):
        if (not self.lastBrokenCandle is None 
            and self.windowMinute[1].Low < self.vwap.Current.Value):
            self.lastBrokenCandle = None
        if (self.lastBrokenCandle is None
            and self.IsPositiveBrokenCandle(self.windowMinute[1])):
            self.lastBrokenCandle = self.windowMinute[1]
